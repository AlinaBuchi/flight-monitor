...
class MongoConnector(Singleton, StorageObject):
    ...
    def read(self, collection_name: str, search_params: dict = None) -> Union[List[dict], dict]:
        collection = self.select_collection(collection_name)
        if search_params:
    ...
    def update_one(self, collection_name: str, search_params: dict, to_update: dict) -> None:
        collection = self.select_collection(collection_name)
        collection.update_one(search_params, {'$set': to_update})

    def delete_one(self, collection_name: str, search_params: dict) -> None:
        collection = self.select_collection(collection_name)
        collection.delete_one(search_params)
