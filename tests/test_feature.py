...
def test_arrival_depart_domestic(my_airport, domestic_plane):
    ...
    assert my_airport.departures_queue.qsize() == 1
    ...
    for gate in my_airport.gates:
        if gate.number == domestic_plane.gate_number:
            assert gate.get_occupied() is False
...
def test_arrival_depart_international(my_airport, international_plane):
    ...
    for gate in my_airport.gates:
        if gate.number == international_plane.gate_number:
            assert gate.get_occupied() is True
...
def test_arrival_repair(my_airport, repair_plane_1):
    ...
    assert len(my_airport.arrivals) == 1
    assert my_airport.departures_queue.qsize() == 0
